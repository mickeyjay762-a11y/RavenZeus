...
def _api_request(url, method="GET", data=None, headers=None, timeout=15):
    ...
    try:
        ...
        try:
            ...
            return {"error": error_body.get("detail", str(e)), "status_code": e.code, "type": "HTTPError"}
        except Exception:
            return {"error": str(e), "status_code": e.code, "type": "HTTPError"}
    except URLError as e:
        # More detailed connection error messages
        error_msg = str(e.reason)
        if "Connection refused" in error_msg:
            return {"error": "Connection refused - Binance API may be blocking requests", "type": "ConnectionRefused"}
        elif "Name or service not known" in error_msg:
            return {"error": "DNS resolution failed - check internet connection", "type": "DNSFailure"}
        elif "timed out" in error_msg.lower():
            return {"error": "Connection timed out - slow or unstable internet", "type": "Timeout"}
        elif "Network is unreachable" in error_msg:
            return {"error": "Network unreachable - check internet connection", "type": "NetworkUnreachable"}
        else:
            return {"error": f"Connection error: {e.reason}", "type": "URLError"}
    except TimeoutError as e:
        return {"error": f"Request timeout after {timeout}s - slow internet or API issues", "type": "TimeoutError"}
    except json.JSONDecodeError as e:
        return {"error": f"Invalid JSON response from API: {str(e)}", "type": "JSONDecodeError"}
    except Exception as e:
        return {"error": f"Unexpected error: {str(e)}", "type": "UnknownError"}
...
def get_binance_momentum(symbol="BTCUSDT", lookback_minutes=5, max_retries=2):
    """Get price momentum from Binance public API.
    Returns: {momentum_pct, direction, price_now, price_then, avg_volume, candles}
    """
    import time
    
    # Try multiple Binance endpoints as fallbacks
    endpoints = [
    ...
    for endpoint_idx, base_url in enumerate(endpoints):
        for attempt in range(max_retries):
            url = f"{base_url}?symbol={symbol}&interval=1m&limit={lookback_minutes}"
            result = _api_request(url)
            
            # Enhanced error handling
            if not result:
                print(f"❌ Binance API (endpoint {endpoint_idx + 1}): No response for {symbol} (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(1)  # Wait 1 second before retry
                    continue
                # Try next endpoint if all retries failed
                break
            
            if isinstance(result, dict) and result.get("error"):
                error_type = result.get("type", "Unknown")
                error_msg = result.get("error", "Unknown error")
                print(f"❌ Binance API Error ({error_type}): {error_msg} (endpoint {endpoint_idx + 1}, attempt {attempt + 1}/{max_retries})")
                
                # Retry on certain types of errors
                if error_type in ["Timeout", "TimeoutError", "URLError"] and attempt < max_retries - 1:
                    time.sleep(2)  # Wait longer for network issues
                    continue
                # Try next endpoint if all retries failed
                break
            ...
            try:
                ...
                candles = result
                if len(candles) < 2:
                    print(f"⚠️  Binance API: Insufficient data (only {len(candles)} candles)")
                    return None
                ...
                print(f"❌ Binance API: Data parsing error - {e} (endpoint {endpoint_idx + 1}, attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(1)
                    continue
                # Try next endpoint if all retries failed
    ...
    print("❌ All Binance API endpoints failed")
    return None
